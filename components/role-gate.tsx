...
type RoleGateProps = {
  ...
  allow?: Array<'student' | 'faculty' | 'admin' | 'guest'>;
};

/** Blocks faculty/admin from student exam UI when allow is student-only. */
export function RoleGate({ children, allow = ['student', 'guest'] }: RoleGateProps) {
  ...
  if ((role === 'faculty' || role === 'admin') && !allow.includes(role)) {
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        <h2 className="text-xl font-bold text-slate-900 mb-2">Student exams only</h2>
        <p className="text-slate-600 mb-6">
          Faculty and administrators use their own portal. You cannot take practice tests from the
          student interface.
        </p>
