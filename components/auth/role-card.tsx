...
export function RoleCard({ href, title, description, icon: Icon }: Props) {
  return (
    <Link
      href={href}
      className={cn(
        'group relative flex items-center gap-4 rounded-xl border border-slate-200/80 bg-white p-4 sm:p-5',
        'transition-all duration-200 hover:border-[#1e3a5f]/35 hover:-translate-y-0.5 hover:shadow-md',
        'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1e3a5f]/40 focus-visible:ring-offset-2',
      )}
    >
      <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-[#1e3a5f]/10 to-[#1e3a5f]/5 text-[#1e3a5f] ring-1 ring-[#1e3a5f]/10">
