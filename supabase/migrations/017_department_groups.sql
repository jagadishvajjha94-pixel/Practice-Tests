-- Department groups: bundle branches so exams reach the right students
-- and faculty in those branches can track progress.
-- Safe to re-run.
...
ALTER TABLE public.faculty_exam_requests
  ADD COLUMN IF NOT EXISTS department_group_id UUID REFERENCES public.department_groups(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_faculty_exam_requests_group
  ON public.faculty_exam_requests(department_group_id);

-- Default groups (RCEE branches — https://www.rcee.ac.in/)
